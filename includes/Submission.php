...
class Submission
{
    ...
    {
        $stmt = db()->prepare(
            'SELECT s.*, d.department_name, d.staff_name
             FROM submissions s
    ...
    public static function search(array $filters, int $page = 1, int $perPage = 10): array
    {
        ...
        if (!empty($filters['search'])) {
            $where[] = '(s.reference_number LIKE ? OR s.submitted_by LIKE ? OR s.email LIKE ? OR d.department_name LIKE ?)';
            $q = '%' . $filters['search'] . '%';
        ...
        $sql = "SELECT s.*, d.department_name, d.staff_name
                FROM submissions s
                JOIN departments d ON d.id = s.department_id
                WHERE $whereSql
                ORDER BY s.submission_date DESC
    ...
    public static function create(array $data, ?string $documentPath): int
    {
        $sql = 'INSERT INTO submissions (
            reference_number, department_id, submitted_by, designation, email, phone,
            ...
            infrastructure_development, community_services, future_plans, supporting_documents,
    ...
    public static function update(int $id, array $data, ?string $documentPath): void
    {
        $existing = self::find($id);
        $doc = $documentPath ?? ($existing['supporting_documents'] ?? null);

        $sql = 'UPDATE submissions SET
            department_id=?, submitted_by=?, designation=?, email=?, phone=?,
            student_growth=?, department_growth=?, special_achievements=?, events_conducted=?,
            income_generation=?, industry_partnerships=?, research_innovations=?, staff_development=?,
            infrastructure_development=?, community_services=?, future_plans=?, supporting_documents=?,
            status=?, updated_at=?
